...
class LocalFunctionReader:
    ...
    def parse_and_basic_validation(self, basedir, module_details, module_name,
        ...
        if os.path.isdir(os.path.join(basedir, 'ui', 'local_functions')):
            for spec in os.listdir(
                    os.path.join(basedir, 'ui', 'local_functions')):
                if os.path.isfile(
                        os.path.join(basedir, 'ui', 'local_functions', spec)):
                    ...
                    if file_name_tokens[1] == 'json':
                        ...
                        if 'tags' in spec_parse:
                            ...
                            if 'categories' in spec_parse:
                                if self._validate_as_list_of_strings(
                                    ...
                                    tags['categories'] = spec_parse['tags'][
                                        'categories']
                                else:
                                    report['functions_errored'].append(
                                        {'filename': spec,
                            ...
                            if 'input' in spec_parse['tags']:
                                if not isinstance(spec_parse['tags']['input'],
                                                  dict):
                                    report['functions_errored'].append(
                                        {'filename': spec,
                            ...
                            if 'output' in spec_parse['tags']:
                                if not isinstance(spec_parse['tags']['output'],
                                                  dict):
                                    report['functions_errored'].append(
                                        {'filename': spec,
                                ...
                                if 'kb_types' in spec_parse['tags']['output']:
                                    if self._validate_as_list_of_strings(
                                            spec_parse['tags']['output'][
                                                'kb_types']):
                                        tags['output']['kb_types'] = \
                                        spec_parse['tags']['output'][
                                            'kb_types']
                                    else:
                                        report['functions_errored'].append(
                                        ...
                                        continue
                                if 'file_types' in spec_parse['tags'][
                                    'output']:
                                    if self._validate_as_list_of_strings(
                                            spec_parse['tags']['output'][
                                                'file_types']):
                        ...
                        if os.path.isfile(
                            ...
                            try:
                                ...
                                report['functions_errored'].append(
                                    {'filename': spec,
                                     'error': "Unable to read file specified in long description: " + str(
                                         e)})
                                continue
    ...
    def report_to_string_for_log(self, report):
        ...
        for r in report['functions_errored']:
            report_str += '    - Error in ui/local_functions/' + r[
                'filename'] + ':\n'
            report_str += '         ' + r['error'] + '\n'
    ...
    def finish_validation(self, compilation_report):
        ...
        if 'functions' not in compilation_report:
            raise ValueError(
                'Invalid compilation report, functions are missing')
        if 'function_places' not in compilation_report:
            raise ValueError(
                'Invalid compilation report, function_places are missing')
        ...
        for fid in self.function_specs:
            if fid not in compilation_report['functions']:
                raise ValueError(
                    'Invalid function specification, "' + fid + '" function could not be found.  Check local function spec names.')
